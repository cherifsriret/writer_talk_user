...
<div class="w-40 mt-6 mx-auto pb-3 bg-white">
	<div class="">
		...
		<div class="request-list mt-2">
			<div class="media py-2"> <img class="mr-3 request-user" src="{{asset('storage/'.$request->user_details->favorite_genres)}}" alt="Generic placeholder image">
				<div class="media-body">
					<h5 class="mt-0 main-name">{{$request->user_details->name}}</h5>
					<div class="time last-seen">{{$request->user_details->favorite_genres}}</div>
					<div class="confirm-btn mb-1 mt-2">
                        ...
                        <form id="accept-request" action="{{ url('update-penpal-status') }}" method="POST" class="d-none">
                            ...
                            <input type="hidden" name="other_user_id" value="{{$request->user_details->uuid}}">
                        </form>
